...
stripe_2025 <- readRDS(here(
  "DataProcessed/experimental/2025/stripe_clean_2025.rds"
)) %>%
  st_as_sf(coords = c("east", "north"))

## Inoculations
inocs_2024 <- readRDS(here("DataProcessed/experimental/inoc_sp.rds"))
inocs_2025 <- readRDS(here(
  "DataProcessed/experimental/2025/inocs_2025.rds"
)) %>%
  st_as_sf(coords = c("east", "north"))

## Grids
design_2024 <- readRDS(here("DataProcessed/experimental/grids_sp.rds"))
design_2025 <- readRDS(here(
  "DataProcessed/experimental/2025/grids_sp_2025.rds"
))

# Read in results ---------------------------------------------------------
## Forward Model
forward_2024 <- readRDS(here(
  "DataProcessed/results/forward_model/forward_fits.rds"
))
forward_2025 <- readRDS(here(
  "DataProcessed/results/forward_model/forward_fits_2025.rds"
))

## Backward Model
backward_2024 <- readRDS(here(
  "DataProcessed/results/backward_model/archived/backward_fits_full.rds"
))
...
## Set plotting colors
intensity_cols <- viridis::viridis(n = 10)
...
stripe_2024 %>%
  ...
  geom_sf(
    data = inocs_2024 %>% filter(block == "A"),
...
ggsave(
  "disease_status.png",
  path = here("Reports/EEID_Presentation/figures/"),
  width = 6,
  height = 8,
  units = "in"
)
...
forward_2024_plt <- forward_2024 %>%
  ...
  left_join(
    stripe_2024 %>% select(block, treat, visit, plant_id, intensity),
    by = c("block", "treat", "visit", "plant_id")
  ) %>%
  select(block, treat, visit, plant_id, fitted, intensity, geometry) %>%
  rename("Predicted" = "fitted", "True" = "intensity") %>%
  pivot_longer(
    c("Predicted", "True"),
    names_to = "type",
    values_to = "intensity"
...
for (blk in unique(forward_2024_plt$block)) {
  for (trt in unique(forward_2024_plt$treat)) {
    forward_2024_plt %>%
      ...
      geom_sf(
        data = inocs_2024 %>% filter(block == blk, treat == trt),
        aes(fill = "Inoculation Point"),
        shape = 23,
        size = 2,
        alpha = 0.75
      ...
      labs(
        ...
        color = "Intensity",
        fill = "",
        title = paste0("2024 Fitted Values (Block: ", blk, " Treat: ", trt, ")")
      ) +
      scale_colour_gradientn(colours = intensity_cols, limits = c(0, 1)) +
      theme(legend.position = "bottom")

    ggsave(
      paste0("forward_preds_2024_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/preds"),
      width = 8,
      height = 5,
      units = "in"
    )
...
forward_2025_plt <- forward_2025 %>%
  ...
  mutate(plant_id = as.numeric(plant_id)) %>%
  left_join(
    stripe_2025 %>% select(block, treat, visit, plant_id, intensity),
    by = c("block", "treat", "visit", "plant_id")
  ) %>%
  select(block, treat, visit, plant_id, fitted, intensity, geometry) %>%
  rename("Predicted" = "fitted", "True" = "intensity") %>%
...
for (blk in unique(forward_2025_plt$block)) {
  for (trt in unique(forward_2025_plt$treat)) {
    forward_2025_plt %>%
      ...
      geom_sf(aes(color = intensity)) +
      geom_sf(
        data = inocs_2025 %>% filter(block == blk, treat == trt),
        aes(fill = "Inoculation Point"),
        shape = 23,
        size = 2,
        alpha = 0.75
      ...
      labs(
        ...
        color = "Intensity",
        fill = "",
        title = paste0("2025 Fitted Values (Block: ", blk, " Treat: ", trt, ")")
      ) +
      scale_colour_gradientn(colours = intensity_cols, limits = c(0, 1)) +
      theme(legend.position = "bottom")

    ggsave(
      paste0("forward_preds_2025_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/preds"),
      width = 8,
      height = 5,
      units = "in"
  ...
  for (trt in unique(forward_2024_plt$treat)) {
    forward_2024_resid %>%
      ...
      labs(
        x = "Fitted",
        y = "Dev. Residual",
        title = paste0("2024 Residuals (Block: ", blk, " Treat: ", trt, ")")
      ) +
      lims(x = c(0, 1)) +
      theme_bw()

    ggsave(
      paste0("forward_resid_2024_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/residual/"),
      width = 6,
      height = 3,
      units = "in"
    )
  ...
  for (trt in unique(forward_2025_plt$treat)) {
    forward_2025_resid %>%
      ...
      labs(
        x = "Fitted",
        y = "Dev. Residual",
        title = paste0("2025 Residuals (Block: ", blk, " Treat: ", trt, ")")
      ) +
      lims(x = c(0, 1)) +
      theme_bw()

    ggsave(
      paste0("forward_resid_2025_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/residual/"),
      width = 6,
      height = 3,
      units = "in"
...
for (blk in unique(forward_2024_plt$block)) {
  for (trt in unique(forward_2024_plt$treat)) {
    forward_2024_resid %>%
      ...
      geom_sf_label(
        data = forward_2024_resid %>%
          filter(block == blk, treat == trt, abs(resid) > 3),
        aes(label = plant_id),
        size = 2,
        nudge_x = 1.5,
        nudge_y = -1.5
      ...
      scale_colour_gradient(
        low = intensity_cols[1],
        high = intensity_cols[length(intensity_cols)],
        limits = c(-3, 3)
      ) +
      theme(legend.position = "bottom")

    ggsave(
      paste0("forward_spat_resid_2024_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/residual/"),
      width = 8,
      height = 5,
      units = "in"
...
for (blk in unique(forward_2025_plt$block)) {
  for (trt in unique(forward_2025_plt$treat)) {
    ...
    ggsave(
      paste0("forward_spat_resid_2025_", blk, "_", trt, ".png"),
      path = here("Reports/EEID_Presentation/figures/forward/residual/"),
      width = 8,
      height = 5,
      units = "in"
...
params_2024 <- forward_2024 %>%
  ...
  mutate(
    value = case_when(param == "phi" ~ exp(value), T ~ value),
    param = case_when(
...
params_2024 %>%
  ...
  labs(
    ...
    y = "Estimate",
    color = "Parameter",
    title = "2024 Parameter Estimates"
  ) +
  theme(legend.position = "bottom")

ggsave(
  filename = "parameter_estimates_2024.png",
  path = here("Reports/EEID_Presentation/figures/forward/"),
  width = 8,
  height = 5,
  units = "in"
)
...
params_2025 %>%
  ...
  labs(
    ...
    y = "Estimate",
    color = "Parameter",
    title = "2025 Parameter Estimates"
  ) +
  theme(legend.position = "bottom")

ggsave(
  filename = "parameter_estimates_2025.png",
  path = here("Reports/EEID_Presentation/figures/forward/"),
  width = 8,
  height = 5,
  units = "in"
...
grids_2024 %>%
  ...
  geom_sf(
    data = inocs_2024 %>% filter(block == "D"),
...
ggsave(
  filename = "experimental_design_2024_blockB.png",
  path = here("Reports/EEID_Presentation/figures/experimental/"),
  width = 8,
  height = 6,
  units = "in"
)
...
grids_2025 %>%
  ...
  geom_sf(
    data = inocs_2025 %>% filter(treat == "single"),
    shape = 23,
    aes(fill = block)
  ) +
  labs(
    x = "East (m)",
    y = "North (m)",
    title = "2025 Experimental Design (Single Inoculation)",
    fill = "Inoculation Point (Block)"
  ) +
...
ggsave(
  filename = "experimental_design_2025.png",
  path = here("Reports/EEID_Presentation/figures/experimental/"),
  width = 8,
  height = 6,
  units = "in"
...
grids_2025 %>%
  ...
  geom_sf_text(aes(label = grid_id), size = 1.5) +
  geom_sf(
    data = inocs_2025 %>% filter(treat == "mixed"),
    shape = 23,
    aes(fill = block)
  ) +
  labs(
    ...
    p_long = map(
      ...
      ~ {
        ...
        df %>%
          ...
          mutate(
            infected_component = as.character(infected_component),
            source_component = as.character(source_component)
          )
      }
    )
  ) %>%
...
spatial_probs_2024 <- left_join(
  backward_long_2024,
  grids_2024 %>%
    mutate(,
      grid_id = as.character(grid_id)
    ),
  by = c("config", "infected_component" = "grid_id"),
  relationship = "many-to-many"
)

saveRDS(
  spatial_probs_2024,
  here("DataProcessed/results/backward_model/network_rep.rds")
)
nodes_2024 <- spatial_probs_2024 %>%
  filter(infected_component == source_component) %>%
  st_as_sf()
...
edges_2024 <- left_join(
  spatial_probs_2024 %>%
    select(-geometry) %>%
    filter(infected_component != source_component),
  centroids_2024,
  by = c("config", "source_component" = "grid_id")
) %>%
...
spatplot <- function(cfg, blk, trt, nodes, edges, grid, inocs) {
  ...
  ggplot() +
    ...
    geom_sf_label(
      data = nodes_plt,
      aes(fill = prob, label = infected_component)
    ) +
    geom_sf(
      data = edges_plt,
      aes(alpha = prob),
...
for (letter in unique(spatial_probs_2024$block)) {
  for (k in unique(spatial_probs_2024$config)) {
    ...
    ggsave(
      plt,
      filename = paste0("spatplot_2024_", letter, k, ".png"),
      path = here(
        "Reports/EEID_Presentation/figures/backward/SpatialPlots/2024/"
      ),
      width = 10,
      height = 4,
      units = "in"
...
spatial_probs_2025 <- left_join(
  backward_long_2025,
  grids_2025 %>% mutate(grid_id = as.character(grid_id)),
  by = c("config", "infected_component" = "grid_id"),
  relationship = "many-to-many"
)

nodes_2025 <- spatial_probs_2025 %>%
  filter(infected_component == source_component) %>%
  st_as_sf()
...
for (letter in unique(spatial_probs_2025$block)) {
  for (k in unique(spatial_probs_2025$config)) {
    ...
    ggsave(
      plt,
      filename = paste0("spatplot_2025_", letter, k, ".png"),
      path = here(
        "Reports/EEID_Presentation/figures/backward/SpatialPlots/2025/"
      ),
      width = 10,
      height = 4,
      units = "in"
...
ggplot() +
  geom_sf(data = stripe_2024 %>% filter(block == "C"), aes(color = intensity)) +
  geom_sf(
  ...
  labs(x = "East (m)", y = "North (m)", fill = "", color = "Intensity") +
  scale_color_viridis_c(option = "D", limits = c(0, 1)) +
  scale_fill_manual(values = c("Inoculation Point" = "#00E5FF")) +
  theme_bw()

ggsave(
  filename = "rawdata_blockC_2024.png",
  path = here("Reports/EEID_Presentation/figures/experimental/"),
  width = 10,
  height = 5,
  units = "in"
...
ggplot() +
  geom_sf(data = stripe_2025 %>% filter(block == "B"), aes(fill = intensity)) +
  geom_sf(
    data = inocs_2025 %>% filter(block == "B"),
    aes(fill = "Inoculation Point"),
    shape = 23,
    size = 3
  ) +
...
ggsave(
  filename = "rawdata_blockA_2025.png",
  path = here("Reports/EEID_Presentation/figures/experimental/"),
  width = 10,
  height = 5,
  units = "in"
...
backward_2024 %>%
  mutate(
    accuracy = if_else(predicted_source == true_source, 1, 0),
    config = factor(
      config,
...
pivot_wider(names_from = init, values_from = c(dist_acc, acc, dist_error)) %>%
  select(
    visit,
    starts_with("acc"),
    starts_with("dist_acc"),
    starts_with("dist_error")
  ) %>%
  kable(
    #format = "latex",
    booktabs = TRUE,
...
discrete_cols <- wesanderson::wes_palette(
  "Darjeeling1",
  n = 5,
  type = "discrete"
)
...
sims_clean %>%
  ...
  group_by(config, visit) %>%
  summarise(
    "acc" = mean(predicted_source == true_source),
    "dist_acc" = mean(dist_acc),
...
backward_2024 |>
  ...
  pivot_wider(
    ...
    values_from = c("error", "acc", "dist_acc")
  ) |>
  ungroup() |>
  mutate(
    group_label = paste0(
  ...
  tab_spanner(
    columns = contains("dist_acc"),
    label = "Distance-Weighted Acc."
  ) |>
