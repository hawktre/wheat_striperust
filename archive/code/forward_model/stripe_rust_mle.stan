...
transformed data {
  ...
  real sd_auto = sd(auto_raw);
  vector[N] auto_scaled = auto_raw / sd_auto;

  // 2. Compute static scale for dispersal at a reference kappa (inverse-square)
  vector[N] dispersal_ref = calc_dispersal(dist_mat, wind_mat, y_lag, 1.0);
  real sd_disp_global = sd(dispersal_ref);
}
...
transformed parameters {
  ...
  // Scale it by the invariant global standard deviation
  vector[N] disp_scaled = dispersal_raw / sd_disp_global;
  
  // Isotropic linear predictor
  logit_mu = beta + delta * auto_scaled + gamma * disp_scaled;
