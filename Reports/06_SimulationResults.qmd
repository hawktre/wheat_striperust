...
In the forward model, we are primarily concerned with bias in the parameter estimates. @fig-forward-bias shows the distribution of parameter estimates across all simulations, stratified by block, treatment-level, and visit. The colored point indicates the true value of the parameter.
...
forward_sims_converged <- forward_sims %>% filter(converged.forward == T) %>% arrange(neg_loglik.forward)
...
What we see in @fig-forward-bias is that the model is able to consistently ree parameter values with some bias, particularly for the $\kappa$ parameter. The $\phi$ parameter is also estimated with some bias, but this is expected as it is a nuisance parameter. The $\beta$ and $\delta$ parameters are estimated with less bias, and the model appears to be able to recover the true values across all blocks, treatment-levels, and visits.ith small bias for all parameters except for $\gamma$, the parameter governing the effect of the dispersal kernel on disease intensity. It is possible that this behavior is due to the model selection strategy, in which we optimize for 10 proposed initialization values of $\kappa$ and select the fit with the smallest negative log-likelihood. This approach may favor less biased values of $\hat\kappa$ at the expense of large values of $\hat\gamma$. Plots for the remaining blocks can be found in @sec-appendix.
