...
We are supplied with wind speed ($m/s$) and direction (degrees azimuth) for the duration of the study period in addition to the $x,y$ coordinates of each sample location, as mentioned previously. First, we compute a *direction* matrix $\boldsymbol\psi$ where each component $\psi_{ij}$ denotes the direction from target $i$ to source $j$ in radians. That is,

$$
\psi_{ij} = \mathrm{atan2}\bigpar{y_j - y_i, x_j - x_i}
$$

Note that the upper-triangle of this matrix will give the direction from source $j$ to target $i$. Now, given the wind speed and direction, we first aggregate wind directions into bins for each of the 16 cardinal directions:
...
Then we follow the process outlined by @gent2019:
  ...
  2. Letting $\boldsymbol{\alpha} = \{\alpha_k\}$ denote the vector of midpoints (in radians) for the 16 cardinal wind directions, define the vector of angular differences relative to the direction from location $i$ to location $j$, denoted $\psi_{ij}$:
  $$
