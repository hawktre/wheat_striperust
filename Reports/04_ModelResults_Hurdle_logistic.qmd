...
$$
\begin{split}
E\left[y_{i,t}|Z_{i,t}\right] &= 0 \times P(Z_{i,t} = 0) + P(Z_{i,t} = 1) \times E\left[y_{i,t}\right] \\
&= (1 - \pi_t) \times \mu_{i,t}
\end{split}
$$
...
From @fig-resid, we see no particular pattern in the residuals at visits 2–4. Notice, however, that we are now systematically *underestimating* disease intensity in Block B at Visit 5, once the plot has become relatively saturated with disease, across all treatment levels. This suggests a trade-off introduced by using a non-linear autoinfection term: while it captures early epidemic dynamics more realistically, it may limit the model’s ability to represent late-stage disease severity.


# Discussion

This report presents results from a reformulated model that incorporates a hurdle structure to explicitly handle non-diseased plants during early time points and accounts for non-linear growth in autoinfection. The clearest outcome of this strategy is a trade-off between biological interpretability and predictive performance. With both the hurdle model and logistic autoinfection term, we obtain the most biologically plausible parameter estimates to date. However, once plots become saturated with disease (e.g., Visit 5), the model systematically underestimates disease severity. This may reflect a decline of estimated dispersal effects in the late stages of the epidemic, or a mismatch between the logistic assumption and late-stage epidemic dynamics.
