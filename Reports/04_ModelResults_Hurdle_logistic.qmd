...
summary_wide %>%
  ...
  cols_label(
    .list = setNames(
      ...
      object = gsub(".*_", "Visit ", names(summary_wide)[-1])  # clean column names
    )
  )
...
ggsave("WindSummary_Stratified.png", path = here("manuscript/Figures/"), width = 8, height = 8, units = "in")
```
...
plot_df <- stripe |> st_as_sf(coords = c('east','north')) |> 
  left_join(df, by = c("block", "treat", "visit", "plant_id")) %>% 
  select(block, treat, visit, plant_id, intensity, fitted, geometry) %>% 
  filter(visit != 1) |> 
  rename(predicted = fitted,
         true = intensity) |> 
          arrange(block, treat, visit, plant_id) |> 
          pivot_longer(cols = c(true, predicted), names_to = "type", values_to = "intensity")
...
df_fitted_resid <- left_join(df, df_resid, by = c("block", "treat", "visit", "plant_id"))
...
library(ggrepel)
#Create a spatial residual df

df_resid_sp <- left_join(df_resid, stripe, by = c("block", "treat", "visit", "plant_id")) |> 
