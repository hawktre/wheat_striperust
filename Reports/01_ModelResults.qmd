...
@fig-params shows parameter estimates at each transition (e.g. "Visit 2" is the transition from "Visit 1" to "Visit 2"). 

```{r}
#| label: fig-params
#| fig-cap: "Parameter estimes reported for each transition (e.g. 'Visit 2' denotes the transition from 'Visit 1' to 'Visit 2') on the log-odds scale."
#| fig-width: 10
#| fig-height: 8
library(tidyverse)
library(here)
...
## Fits results
fits <- readRDS(here("DataProcessed/results/fits_final.rds"))

##Model Data
mod_dat <- readRDS(here("DataProcessed/results/mod_dat_appended.rds"))
fits <- bind_rows(fits)
...
theta_long %>% 
  ...
  labs(y = "Estimate", 
       x = "Visit", 
       title =expression(bold(theta) ~ "across visits (2-5)")) +
  scale_color_manual(
    name = "Parameter",
...
```{r}
#| label: tbl-params
#| tbl-cap: "Tabular parameter estimes reported for each transition (e.g. 'Visit 2' denotes the transition from 'Visit 1' to 'Visit 2') on the log-odds scale, matching with @fig-params."
fits %>%
...
df <- map_dfr(names(mod_dat), function(plot_id) {
  plot_obj <- mod_dat[[plot_id]]
...
all_fit_results <- readRDS(here("DataProcessed/results/fits_by_gamma_max.rds"))
...
theta_long_all <- map_dfr(
  ...
  mutate(
    ...
    gamma_max = as.numeric(gamma_max)  # convert from character to numeric
  )
