...
$$
\begin{split}
\frac{\partial Q}{\partial\beta} &= \nsum \sum_{k=1}^K\text{grad}_{i,t}^{(k)} \\
\frac{\partial Q}{\partial\delta} &= \nsum \sum_{k=1}^K\text{grad}_{i,t}^{(k)}\times y_{i,t-1}(1 - y_{i,t-1}) \\
\frac{\partial Q}{\partial\gamma} &= \nsum \sum_{k=1}^K\text{grad}_{i,t}^{(k)} \times \sum_{j \in G_s}\bigpar{y_{j,t-1}w_{ij} (d_{ij} + d_0)^{-\kappa}} \\
\frac{\partial Q}{\partial\kappa} &= \nsum \sum_{k=1}^K\text{grad}_{i,t}^{(k)} \times -\gamma \sum_{j \in G_s}\bigpar{y_{j,t-1}w_{ij} (d_{ij} + d_0)^{-\kappa}\log(d_{ij} + d_0)}
\end{split}
$$ {#eq-newgrads}
