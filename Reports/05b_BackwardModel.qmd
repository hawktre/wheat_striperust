...
As in @ojwang2021, Let us introduce a latent process $Z$ which indicates the causal attribution of disease to one of $k \in \bigpar{1, \ldots, K}$ sources with a certain probability:
...
Define the complete data as $\mathcal{X} = \bigbrace{\mathcal{Y}, \boldsymbol{Z}}$, where $\mathcal{Y} = \bigbrace{\mathbf{y}_t, \mathbf{y}_{t-1}, \mathbf{w}_t, \mathbf{d}}$ Our goal here is to find the maximum likelihood estimate of the marginal likelihood of our *observed* data $\mathrm{ZIB}(y_{i,t}; \alpha_t, \mu_{i,t}, \phi_t)$ by iteratively applying two steps: 


$$
\ell(\boldsymbol{\theta} \mid \mathcal{X}) = \sum_{i=1}^n \sum_{k=1}^K \mathbb{1} \{Z_i = k\} \cdot \log f(y_{i,t} \mid \mu_{i,t}^{(k)}, \phi_t)
$$ {#eq-loglik}

where $\mu_{i,t}^{(k)}$ denotes the predicted mean intensity at $i$ assuming infection from source group $s$. We can then apply the Expectation-Maximization (EM) algorithm to estimate the posterior source probabilities $\hat p^{(s)}$. At each iteration:
...
grid <- readRDS(here("DataProcessed/experimental/grids_sp.rds"))

disease_dat <- readRDS(here("DataProcessed/experimental/stripe_clean.rds"))

disease_sp <- disease_dat |> 
  select(north, east) |> 
  distinct() |> 
...
grid %>% 
  bind_rows() |> 
  filter(grid_type == "8 x 8") |> 
  ggplot()+
  geom_sf(fill = "transparent")+
  geom_sf(data = disease_sp) + 
...
mle_results <- readRDS(here("DataProcessed/results/backward_model/backward_fits.rds"))
...
## Compute BIC
fit_df <- mle_results |> 
  mutate(n_mix = parse_number(config),
         bic_k = (n_mix - 1 + 5),
         bic = bic_k * log(64) - 2 * abs(neg_loglik.backward))
...
best_mod <- fit_df %>%
  ...
  ungroup() %>% 
  mutate(configuration = factor(config, 
                                levels = c("4", "8h", "8v", "16"), 
