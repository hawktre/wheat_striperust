...
sp_tbl_short <- function(blk){
  truth_sp_summary %>% 
  filter(block == blk, grid_type == "4 x 4") %>% 
  select(visit, mean_acc, incorrect_preds, mean_error, mean_error_miss) %>% 
  st_drop_geometry() %>% 
  arrange(visit) %>% 
  kable(digits = 3, col.names = c("Visit", "Dist. Acc.", "Incorrect Predicitons", "Error (m)", "Miss Error (m)"), format = "latex") 
}
...
```{r}
truth_sp_summary %>% 
  group_by(grid_type, visit) %>% 
  arrange(grid_type, visit) %>% 
  summarise(mean_acc = mean(mean_acc),
            mean_error = mean(mean_error),
            mean_error_miss = mean(mean_error_miss), .groups = "drop") %>% 
  kable(digits = 4, col.names = c("Configuration", "Visit", "Avg. Distance-weighted Accuracy","Avg. Error (m)", "Avg. Miss Error (m)")) %>% 
