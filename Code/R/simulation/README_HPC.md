...
```bash
bash Code/shell/simulation/submit_whole_epidemic_simulations.sh 1 10 4
```

After checking runtimes and memory use, submit simulations 11 through 1000:

```bash
bash Code/shell/simulation/submit_whole_epidemic_simulations.sh 11 1000 12
```
...
```bash
Rscript --vanilla Code/R/simulation/combine_hpc_simulations.R 1000
```
