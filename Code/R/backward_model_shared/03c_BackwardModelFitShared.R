...
# Fit model
backward_result <- backward_fit(config = combo$config, blk = combo$blk, trt = combo$trt, 
                                vst = combo$vst, n_src = combo$n_src, inits = combo$theta[[1]], 
                                mod_dat = mod_dat, tol = 1e-4, max_iter = 200)

# Get predictions  
if (backward_result$converged && !is.null(backward_result$p_mat)) {
  predictions <- source_pred(config = combo$config, blk = combo$blk, trt = combo$trt, 
                             vst = combo$vst, n_src = combo$n_src, 
                             p_mat = backward_result$p_mat[[1]], mod_dat = mod_dat)
} else {
  predictions <- data.table(config = combo$config, block = combo$blk, treat = combo$trt, 
                            visit = combo$vst, n_src = combo$n_src, mean_error = NA, n_correct = NA, 
                            acc = NA, dist_acc = NA, component_dist_acc = list(NA), 
                            predicted_source = list(NA), true_source = list(NA))
}

# Drop large objects and merge
backward_result <- backward_result[, !c("p_mat", "pi")]
final_result <- merge(backward_result, predictions, by = c("config", "block", "treat", "visit", "n_src"))
...
saveRDS(final_result, file.path(output_dir, paste0("backward_blk", combo$blk,"_trt",combo$trt,
                                                   "_vst",combo$vst,"_config", combo$config, "_nsrc", combo$n_src, "_shared.rds")))
