...
backward_individual <- readRDS(here(
  "DataProcessed/results/backward_model/backward_fits_individual.rds"
))
backward_shared <- readRDS(here(
  "DataProcessed/results/backward_model/backward_fits_shared.rds"
))
backward_sensitivity <- readRDS(here(
  "DataProcessed/results/backward_model/backward_fits_sensitivity_sequential.rds"
))


# Explore shared model results with BIC ----------------------------------
bic_preds <- backward_shared |>
...
backward_individual <- left_join(
  backward_individual %>% select(-p_mat),
  individual_predictions %>% select(-p_bar),
  by = c("config", "block", "treat", "visit", "n_src")
)
backward_shared <- left_join(
  backward_shared %>% select(-p_mat),
  shared_predictions %>% select(-p_bar),
  by = c("config", "block", "treat", "visit", "n_src")
)
backward_sensitivity <- left_join(
  backward_sensitivity %>% select(-p_mat),
  sensitivity_predictions %>% select(-p_bar),
  by = c("config", "block", "treat", "visit", "n_src")
)

backward <- list(
  "individual" = backward_individual,
...
saveRDS(
  backward,
  here("DataProcessed/results/backward_model/all_backward_fits.rds")
)
