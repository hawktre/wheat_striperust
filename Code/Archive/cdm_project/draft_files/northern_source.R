...
# fit ITS model
source1_its <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(rho = rho.s, 
         theta = theta.s,
         g.theta = g.theta.s,
         sinb1 = sinb1.s, 
...
source2_its <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(rho = rho.n, 
         theta = theta.n,
         g.theta = g.theta.n,
         sinb1 = sinb1.n, 
         sinb2 = sinb2.n, 
         response = iresponse.n)
initdata_its <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(z.fit = z.fit.sn,
         z.reg.1 = rho.s,
         z.reg.2 = rho.n)
...
# fit ATS model
source1_ats <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(rho = rho.s, 
         theta = theta.s,
         g.theta = g.theta.s,
         sinb1 = sinb1.s, 
...
source2_ats <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(rho = rho.n, 
         theta = theta.n,
         g.theta = g.theta.n,
         sinb1 = sinb1.n, 
         sinb2 = sinb2.n, 
         response = aresponse.n)
initdata_ats <- filter(sub_data, !(ID %in% c(s_id, n_id))) %>%
  rename(z.fit = z.fit.sn,
         z.reg.1 = z.reg.s,
         z.reg.2 = z.reg.n)
...
betahat_ats2 <- coef(fit_ats_n)
betahat_its1 <- coef(fit_its_s)
betahat_its2 <- coef(fit_its_n)
...
sub_data <- ungroup(sub_data) %>%
  mutate(t.hat.ats.s = (aresponse.s - betahat_ats1[1] - betahat_ats1[3]*sinb1.s - betahat_ats1[4]*sinb2.s)/betahat_ats1[2],
                   t.hat.ats.n = (aresponse.n - betahat_ats2[1] - betahat_ats2[3]*sinb1.n - betahat_ats2[4]*sinb2.n)/betahat_ats2[2],
                   ...
                   t.hat.its.n = (iresponse.n - betahat_its2[1])/betahat_its2[2],
                   ...
                   rho.n.hat.its = (exp(predict(fit_its_n, sub_data)) - 1),
                   rho.s.hat.ats = (exp(predict(fit_ats_s, 
                                                rename(sub_data,
                                                       sinb1 = sinb1.s,
                                                       sinb2 = sinb2.s))) - 1)*g.theta.s,
                   rho.n.hat.ats = (exp(predict(fit_ats_n, 
                                                rename(sub_data,
                                                       sinb1 = sinb1.n,
                                                       sinb2 = sinb2.n))) - 1)*g.theta.n)

# round p_i's and choose predictions
which_pred_ats <- data.frame(which.pred.ats = round(pr_z_ats, 0) + (round(pr_z_ats, 0) == 0)*2,
                             ID = source1_ats$ID)
which_pred_ats2 <- data.frame(which.pred.ats.t = replace_na(sub_data$t.hat.ats.s < sub_data$t.hat.ats.n, T),
                              which.pred.ats.rho = replace_na((sub_data$rho.s.hat.ats - sub_data$rho.s) < (sub_data$rho.n.hat.ats - sub_data$rho.n), T),
                              ID = sub_data$ID)
which_pred_its <- data.frame(which.pred.its = round(pr_z_its, 0) + (round(pr_z_its, 0) == 0)*2,
                             ID = source1_its$ID)
which_pred_its2 <- data.frame(which.pred.its.t = replace_na(sub_data$t.hat.its.s < sub_data$t.hat.its.n, T),
                              which.pred.its.rho = replace_na((sub_data$rho.s.hat.its - sub_data$rho.s) < (sub_data$rho.n.hat.its - sub_data$rho.n), T),
                              ID = sub_data$ID)
...
sub_data_pred[sub_data_pred$ID == n_id, c(5:6, 9:10)] <- NA
...
pred_df <- pred_df %>%
  mutate(pred.ats.s = predict(fit_ats_s, newdata = pred_df),
         pred.ats.n = predict(fit_ats_n, newdata = pred_df),
         pred.its.s = predict(fit_its_s, newdata = pred_df),
         pred.its.n = predict(fit_its_n, newdata = pred_df),
         ...
         g.theta.s = density.circular(x = filter(preprocess_out$wind.s, year == yrs[j])$direction,
                                      z = pred_df$theta, 
                                      bw = 50)$y,
         g.theta.n = density.circular(x = filter(preprocess_out$wind.n, year == yrs[j])$direction,
                                      z = pred_df$theta, 
                                      bw = 50)$y) %>%
  mutate(rho.ats.s = (exp(pred.ats.s) - 1)*g.theta.s,
         rho.ats.n = (exp(pred.ats.n) - 1)*g.theta.n,
         rho.its.s = (exp(pred.its.s) - 1),
         rho.its.n = (exp(pred.its.n) - 1),
...
pred_df <- pred_df %>%
  cbind(ats.s = destPoint(p = source_df[1, 2:3],
        ...
        ats.n = destPoint(p = source_df[2, 2:3],
                          b = pred_df$theta*360/(2*pi),
                          d = 1000*pred_df$rho.ats.n),
        its.s = destPoint(p = source_df[1, 2:3],
                          b = pred_df$theta*360/(2*pi),
                          d = 1000*pred_df$rho.its.s),
        its.n = destPoint(p = source_df[2, 2:3],
                          b = pred_df$theta*360/(2*pi),
                          d = 1000*pred_df$rho.its.n),
