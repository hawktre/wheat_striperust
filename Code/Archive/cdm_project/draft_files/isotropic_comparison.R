...
## ISOTROPIC ONE SOURCE MODEL

# fit model
lmod0 <- lm(response.0 ~ time, 
           data = train_data)
...
sub_data <- mutate(sub_data, 
                   ...
                   t.hat.iso = (response.0 - beta0[1])/beta0[2]) %>%
...
plot(sub_data[train_idx, ]$time, sub_data[train_idx, ]$t.hat.iso,
     xlab = 'Day of year of actual occurrence', ylab = 'Predicted day of year',
     main = paste('Isotropic model fit on training data ', yr),
     xlim = c(100, 220),
     ylim = c(50, 300))
abline(b = 1, a = 0)
abline(b = 1, a = 21, lty = 2, col = 4)
...
plot(sub_data[-train_idx, ]$time, sub_data[-train_idx, ]$t.hat.iso,
     xlab = 'Day of year of actual occurrence', ylab = 'Predicted day of year',
     main = paste('Isotropic model fit on test data ', yr),
     xlim = c(100, 220),
     ylim = c(50, 300))
abline(b = 1, a = 0)
abline(b = 1, a = 21, lty = 2, col = 4)
