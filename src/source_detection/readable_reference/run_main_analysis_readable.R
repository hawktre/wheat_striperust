...
# analysis remains src/source_detection/run_main_analysis.R.
...
source(here(
  "src", "source_detection", "readable_reference",
  "source_detection_functions_readable.R"
))

model_data <- readRDS(here(
  "data", "processed", "experimental", "mod_dat_arrays.rds"
))
...
reference_output_directory <- here(
  "output", "source_detection_readable_reference"
)
