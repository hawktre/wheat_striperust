...
## Experiment Data
stripe_2024 <- readRDS(here("data/processed/experimental/stripe_clean.rds")) %>%
  st_as_sf(coords = c("east", "north"))

## Inoculations
inocs_2024 <- readRDS(here("data/processed/experimental/inoc_sp.rds"))

## Grids
design_2024 <- readRDS(here("data/processed/experimental/grids_sp.rds"))


# Read in Results --------------------------------------------------------
forward <- readRDS(here(
  "archive/output/forward_model/stan_forward_fits_multistart.rds"
))
backward <- readRDS(here(
  "archive/output/backward_model/all_backward_fits.rds"
))
...
params_wide |>
  select(block, treat, visit, beta, delta, gamma, kappa, phi, alpha) |>
  # Make sure it's sorted by block so the grouping works flawlessly
  arrange(block, treat, visit) |> 
  mutate(across(c(beta, delta, gamma, kappa, phi, alpha), format_num)) |>
  kable(
    format = "latex", 
    booktabs = TRUE, 
    label = "par-estimates",
    caption = "blah blah blah", 
    col.names = c("Block", "Treatment", "Visit", "$\\beta$", "$\\delta$", "$\\gamma$", "$\\kappa$", "$\\phi$", "$\\alpha$"),
    escape = FALSE,   
    align = "cccrrrrrr"
  ) |>
...
forward %>%
  ggplot(aes(x = as.numeric(visit), y = estimate)) +
  ...
  facet_grid(
    ...
    labeller = labeller(
      ...
      # Use as_labeller with a list of raw expressions
...
ggsave(
  filename = "parameter_estimates.png",
  path = here("manuscript/Figures/"),
  width = 8,
  height = 5,
  units = "in"
...
acc_diff_estimation <- estimation |>
  ...
  mutate(
    source_type = factor(
      if_else(n_src == 1, "Single-source", "Multi-source"),
...
acc_diff_estimation %>%
  pivot_wider(
    ...
    values_from = c(shared, individual, diff)
  ) %>%
  ...
  kbl(
    ...
    col.names = c(
      "Visit",
      rep(c("Shared", "Component-Specific", "Difference"), 2)
    )
  ) %>%
...
acc_diff_initialization %>%
  pivot_wider(
    names_from = source_type,
    values_from = c(shared, sensitivity, diff)
  ) %>%
  select(visit, contains("Single"), contains("Multi")) %>%
  kbl(
    format = "latex",
...
backward_shared |>
  ...
  labs(y = "Distance-weighted Accuracy", x = "Visit") +
  theme_bw()

ggsave(
  filename = "experiment_boxplot_main.png",
  path = here("manuscript/Figures/"),
  width = 14,
  height = 6,
  units = "in"
...
backward_shared |>
  ...
  labs(y = "Distance-weighted Accuracy", x = "Visit", title = "Multi-source") +
  theme_bw()

ggsave(
  filename = "multi_source_boxplot_main.png",
  path = here("manuscript/Figures/"),
  width = 14,
  height = 4,
  units = "in"
...
backward_shared |>
  ...
  pivot_wider(
    names_from = source_type,
    values_from = c(N, dist_acc, acc, error)
  ) %>%
  select(visit, contains("Single"), contains("Multi")) %>%
  kbl(
    format = "latex",
    ...
    col.names = c(
      "Visit",
      rep(c("N", "Distance-weighted Acc.", "Accuracy", "Error (m)"), 2)
    )
  ) %>%
...
sims <- readRDS(here("archive/output/simulation_legacy/sims.rds"))
...
forward_sims_long <- forward_sims %>%
  ...
  unnest(theta) %>%
  mutate(value = case_when(param == "phi" ~ exp(value), T ~ value))
...
forward_sims_long <- left_join(
  forward_sims_long,
  true_params %>% mutate(visit = as.numeric(visit)),
...
plot_forward_sims <- function(blk, drop_gamma = F) {
  if (drop_gamma) {
    forward_sims_long %>%
      mutate(
        treat_lab = paste0(treat, " Inoculation(s)"),
...
for (blk in LETTERS[1:4]) {
  ...
  ggsave(
    filename = paste0("forward_sims_bias_blk", blk, ".png"),
    path = here("manuscript/Figures/"),
    width = 14,
    height = 6,
    units = "in"
...
for (blk in LETTERS[1:4]) {
  ...
  ggsave(
    filename = paste0("forward_sims_bias_nogamma_blk", blk, ".png"),
    path = here("manuscript/Figures/"),
    width = 10,
    height = 6,
    units = "in"
...
ggsave(
  blkA_simbias,
  filename = "parameter_bias_blkA.png",
  path = here("manuscript/Figures/"),
  width = 14,
  height = 6,
  units = "in"
...
ggsave(
  blkA_simbias_nogamma,
  filename = "parameter_bias_nogamma_blkA.png",
  path = here("manuscript/Figures/"),
  width = 14,
  height = 6,
  units = "in"
...
# Find the empty combos
empty_panels <- backward_sims %>%
  ...
  complete(config, n_src, visit) %>%
  group_by(config, n_src) %>%
  filter(all(is.na(dist_acc))) %>%
  distinct(config, n_src) %>%
  ungroup()
